...
{
    Plotter plotter;
    plotter.sample_count = sample_count;
    plotter.parameter_interval = parameter_interval;
    plotter.epsilon = epsilon;

    _plot = plotter.plot(outline);
}
...
bool PlotCover::is_inside(Point const & p) const
{
    ...
    for(size_t j = 0; j < _plot.size(); j++) 
        for(size_t i = 1; i < _plot[j].size(); i++)
            if(segment_intersects_horizontal_ray(_plot[j][i-1], _plot[j][i], p).first)
                count++;
