...
uniform sampler2D stroke_position;    // vec4(x, y, t, 1)
uniform sampler2D stroke_direction;   // vec4(dx, dy, dt, 1)
...
float magixBox(vec3 p);
float magicBox(vec2 p);
mat2 rot2D(float r);
float nsin(float);
float ncos(float);
vec3 saturate(vec3);
float rand(vec2);
float rand(float);
float dtoa(float, float);
float sdAxisAlignedRect(vec2 uv, vec2 tl, vec2 br);
float sdCircle(vec2 uv, vec2 origin, float radius);
float smoothstep4(float, float, float, float, float);
vec2 hash(vec2);
float noise(in vec2);
float noise01(vec2);
vec3 dtocolor(vec3, float);
float smoothf(float);
vec2 getuv_centerX(vec2 fragCoord, vec2 newTL, vec2 newSize);

vec3 colorBrushStroke(vec2 uvLine, 
                      vec2 uvPaper, 
                      vec2 lineSize, 
                      float sdGeometry, 
                      vec3 inpColor, 
...
vec3 colorBrushStrokeLine(vec2 uv, 
                          vec3 inpColor, 
                          vec4 brushColor, 
                          vec2 p1, 
                          vec2 p2, 
...
vec3 humanizeBrushStrokeDonut(vec2 uvLine, 
                              float radius_, 
                              bool clockwise, 
                              float lineLength);

vec3 colorBrushStrokeDonut(vec2 uv, 
                           vec3 inpColor, 
                           vec4 brushColor, 
                           vec2 o, 
                           float radius_, 
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 mouse = getuv_centerX(iMouse.xy, 
                               vec2(-1,-1), 
                               vec2(2,2));// (iMouse.xy / iResolution.y * 2.0) - 1.;

	vec2 uv = getuv_centerX(fragCoord, 
                            vec2(-1,-1), 
                            vec2(2,2));// 0-1 centered
    ...
    col = colorBrushStrokeLine(uv, 
                               col, 
                               vec4(vec3(.8,.1,0),0.4),// red fixed line
                               vec2(1.3, 0.+yo),
                               vec2(-2.9, 0.+yo), 
                               0.03, 
                               mouse);
    ...
    col = colorBrushStrokeLine(uv, 
                               col, 
                               vec4(vec3(.8,.1,0),0.52),// red fixed line
                               vec2(1.3, .3+yo + (cos(uv.x * 12.) * 0.025)),
                               vec2(-2.9, .3+yo), 
                               0.1, 
                               mouse);

    col = colorBrushStrokeDonut(uv, 
                                col, 
                                stroke_color,
                                vec2(0,0),// origin
                                0.6,// radius
                           ...
                           vec2 mouse)
{
    ...
    vec3 ret = colorBrushStroke(humanizedUVLine, uv, vec2(lineWidth1, lineLength), d, inpColor, brushColor, mouse);
    ...
    if(angle > pi)
    {
        ...
        ret2 = colorBrushStroke(humanizedUVLine, uv, vec2(lineWidth, lineLength), d, inpColor, brushColor, mouse);
	}
    return min(ret, ret2);
}
...
//    uvLine.x += (noise01(uvLine * res * 0.18)-0.5) * 0.0035;// HP random noise makes it look less scientific
    ...
    vec3 ret = colorBrushStroke(vec2(uvLine.x, -uvLine.y), 
                                uv, 
                                lineSize,
                                d, 
                                inpColor, 
...
// vec3 colorBrushStrokeBezier(vec2 uv, 
//                             vec3 inpColor, 
//                             vec4 brushColor, 
//                             vec2 from,
//                             vec2 c1,
...
vec2 getuv_centerX(vec2 fragCoord, 
                   vec2 newTL, 
                   vec2 newSize)
{
